...
**The price elasticity of demand for sardines is -1.545. We can say that there is a 95% chance that the the range of -1.699 to -1.392 contains to true price elasticity value.** 

**We reject the null hypothesis that the price elasticity is equal to -1 because it does not fall within the confidence interval range of -1.699 to -1.392.**

**We reject the Null hypothesis that the price elasticity is equal to -1 due getting a statistically significant p-value (0.000000000002946) by to the linear hypothesis testing.**
...
linearHypothesis(model_1, c("log(price_euro_kg) = -1"), white.adjust = "hc2")
...
**The F-statistic is 144.65, which means the instruments are relevant and non-weak (Lecture 9, slide 14).** 
...
**The two stage least squares TSLS of the price elasticity of demand for sardines using wind_m_s as an instrument for log(price_euro_kg) is -1.08802**

```{r}
# TSLS - JUST-IDENTIFIED MODEL
# Lecture 8, slide 13
tsls1 <- ivreg(log(volume_sold_kg) ~ log(price_euro_kg) | wind_m_s, data = EU_sardines_data)
...
# TSLS - JUST-IDENTIFIED MODEL
# Lecture 8, slide 13
tsls2 <- ivreg(log(volume_sold_kg) ~ log(price_euro_kg) + 
